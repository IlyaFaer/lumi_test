...
- I think when we're sending entries, we may send one entry for both debit and credit. It'll have `from_account`, `to_account` and only one sum. This will allow to omit sum validation operation and will prevent many mistakes. In the database, we can still write them as 2 separate rows.
